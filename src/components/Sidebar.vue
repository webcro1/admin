...
import { HomeIcon, Cog6ToothIcon, ClockIcon, ChevronUpDownIcon, ArrowLeftOnRectangleIcon, Bars3Icon } from '@heroicons/vue/24/outline'
...
function onClickOutside(event) {
  if (userDropdownOpen.value && dropdownRef.value && !dropdownRef.value.contains(event.target)) {
    userDropdownOpen.value = false
  }
}
  ...
  <div>
    ...
    >
      ...
      <div class="flex items-center gap-3 p-4 border-b border-[#27272a]">
        <img src="/webcro.png" alt="logo" class="w-10 h-10 rounded-md object-cover" />
        <div>
      ...
      <nav class="flex-1 px-2 py-4 overflow-y-auto space-y-1">
       <!-- <router-link to="/dashboard" class="flex items-center px-3 py-2 rounded hover:bg-[#18181b] transition">
        ...
        -->
        <router-link to="/session" class="flex items-center px-3 py-2 rounded hover:bg-[#18181b] transition">
      ...
      <div class="border-t border-[#27272a] p-4 mt-auto relative flex flex-col items-stretch">
        <div ref="dropdownRef" class="flex flex-col items-stretch">
          ...
          >
            ...
            <div class="flex-1 text-left min-w-0">
              <div class="font-medium text-sm truncate">{{ email || 'Utilisateur' }}</div>
              <div class="text-xs text-gray-400 truncate">{{ name }}</div>
            </div>
            <ChevronUpDownIcon class="w-4 h-4 text-gray-400 transition-transform duration-200" :class="{'-rotate-180': userDropdownOpen}" />
          </button>
          <!-- Dropdown -->
          <transition
          ...
          >
            ...
            >
              <div class="flex items-center px-4 py-3 gap-3 border-b border-[#232323]">
                <img
                  :src="avatarUrl"
                  alt="user"
                  class="w-10 h-10 rounded-full object-cover"
                ...
                <div class="min-w-0">
                  ...
                  <div class="text-xs text-gray-400 truncate">{{ email || 'Utilisateur' }}</div>
                </div>
              </div>
              <button
                @click="logout"
